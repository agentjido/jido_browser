defmodule JidoBrowser.Adapters.VibiumIntegrationTest do
  ...
  describe "navigate/3" do
    test "fetches a real webpage", context do
  ...
  describe "click/3" do
    test "clicks an element on the page", context do
      if session = context[:session] do
  ...
  describe "screenshot/2" do
    test "captures a screenshot", context do
      if session = context[:session] do
        {:ok, session, _nav_result} = Vibium.navigate(session, "https://example.com", [])
  ...
  describe "extract_content/2" do
    test "extracts page content", context do
      if session = context[:session] do
        {:ok, session, _nav_result} = Vibium.navigate(session, "https://example.com", [])
  ...
  describe "type/4" do
    test "types text into a search input", context do
      if session = context[:session] do
